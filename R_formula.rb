class RFormula < Formula
  ...
  module_commands do
    ...
    commands = [ "unload #{pe}-gnu #{pe}-pgi #{pe}-intel #{pe}-cray" ]
    commands << "unload PE-gnu PE-pgi PE-intel PE-cray"
    commands << "unload r"
    commands << "load PE-gnu"
    commands << "load szip"
    commands << "load acml/5.3.1"
    commands << "load sprng"
    commands << "load netcdf-parallel"
    commands << "load zeromq"
    commands
  end
  ...
  def install
    ...
    if module_is_available?("acml/5.3.1")
      acml_prefix = module_environment_variable("acml/5.3.1", "ACML_DIR")
